...
* Section 4.2: added `transform_awaiter` code and walkthrough showing how the promise's `await_transform` bridges the compiler's one-argument `await_suspend` to the protocol's two-argument form and propagates the environment from parent to child. Added reference to the `io_awaitable_promise_base` mixin (P4172R0 Appendix C).
