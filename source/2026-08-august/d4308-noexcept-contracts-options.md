...
A C++26 contract-violation handler may throw. [P3100R8](https://www.open-std.org/jtc1/sc22/wg21/docs/papers/2026/p3100r8.pdf) adds implicit contracts to cover language UB. That creates a narrow but important design question: what should happen if the handler throws while evaluating an ordinary core-language expression that `noexcept` still reports as non-throwing?

P3100R8 defines four responses (Options A-D) and proposes A.<sup>[1]</sup> This paper reopens the full response space by restoring Option 0 (the value-reporting `noexcept` model from [P3541R1](https://www.open-std.org/jtc1/sc22/wg21/docs/papers/2025/p3541r1.html) Option 1<sup>[6]</sup>), which is the only option in the paper which changes the `noexcept` operator, and by adding three further options (E-G).

This paper does not attempt to settle the broader C++29 question of non-throwing semantics for Contracts generally.

---

## 3. Premise

Changing the operator's answer would be a source-level breaking change. The familiar case is code that dispatches on `noexcept(e)`, such as a standard-library move operation that falls back to the throwing-copy path when the move is potentially throwing. The SG21 poll record on P3541R1 points this way: the value-reporting option was rejected in favor of not changing the result (Table 3, Polls 1 and 4).<sup>[6]</sup><sup>[7]</sup>

There is no study of how much code depends on this premise. A well-defined program can change behavior when such a `noexcept` value changes, but that does not imply that the affected code would necessarily malfunction. The premise therefore rests partially on evidence that is not on the record.

This premise excludes Option 1 of [P3541R1](https://www.open-std.org/jtc1/sc22/wg21/docs/papers/2025/p3541r1.html)<sup>[6]</sup>, the option SG21 considered in Poll 1. This paper attempts to evaluate that option as well.

Two questions travel together here, and the paper separates them. One is whether C++29 Contracts should adopt non-throwing evaluation semantics in general, for explicit assertions as well as implicit ones. The other is what response model P3100R8's implicit assertions should use when a handler throws. This paper is about the second question. It takes no position on the first beyond noting, for each option, whether that option would also answer it.

That scope boundary matters because the recovery case is different across the two classes. An explicit, author-written precondition can express a recoverable condition, so continuation there can be coherent. An implicit assertion fires at a point the language has left undefined. The recovery argument for a throwing response is therefore strongest for the explicit assertions this paper excludes. The present enumeration neither relies on that argument nor forecloses it for the implicit assertions it does cover.
...
Second, the unconditional form avoids the link break only by making the operator return `false` for such an expression in every build, whether or not this build can throw. That escapes the ODR problem by paying the source-level price Section 3 describes: `noexcept(x + 1)`, `noexcept(*p)`, and `noexcept(a[i])` flip from `true` to `false` for nearly every scalar and pointer expression in the language, the mass source-level change Section 3 described. The flip does not stop at expressions written by hand: it hollows out the standard library's noexcept-querying type traits, so that `std::is_nothrow_move_assignable_v<S>` reports `false` even for a trivial `struct S {}`, because the move-assignment expression it forms could touch an object outside its lifetime, and nearly every such trait changes its answer for the same reason. It also runs against the C++26 design principle directly: P2900R14 Principle 3 states that "the mere presence of a contract assertion ... should not change ... the value returned by the noexcept operator,"<sup>[3]</sup> and the unconditional form is that change.
...
The value-reporting way to let an implicit assertion throw fails on both forms: the build-mode-dependent operator breaks linking, the unconditional operator is the mass source-level change Section 3 describes and the principle forbids, and SG21 polled the option down. The value-preserving way, Option A, remains available, and reaches an escaping exception by holding the operator's value while shifting its meaning (Section 5.2). With the value-reporting operator removed, the question the enumeration raises is no longer how to report a throwing implicit assertion, but whether an implicit assertion should invoke a throwing handler at all. Options E, F, and G answer that question, and Section 7 presents them.
