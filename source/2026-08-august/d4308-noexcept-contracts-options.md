...
- Cross-reference fixes (Section 3.3/3.2 -> Section 3).
- Incorporates review feedback.
- Editorial compression: contributions list, section leads, conclusion restatement. No change to options or tables.
...
This paper is `info`. It enumerates the response options for one question and states one finding: the response space for this question is larger than the four options currently before EWG.
...
This section defines the terms the rest of the paper uses and states the premise P3100 adopts. A reader fluent in C++26 Contracts can skip to the premise in Section 3.3; the definitions come first because the options differ only in details these terms make precise.
...
Section 3 named the option foreclosed by the premise: letting the `noexcept` operator return `false` for an expression that carries a potentially-throwing implicit assertion. This section restores that option - Option 0 - and gives it the treatment every other option gets: the case for it at full strength, then the costs that removed it from the enumeration. Section 6.3 leaves the remaining question for Section 7: whether an implicit assertion should invoke a throwing handler at all.
...
This section completes Table 2's deployment column, states what Table 1's requirements grid rests on, and then applies five additional dimensions from the public record. The section reports these dimensions and states no ranking.
