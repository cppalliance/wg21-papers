...
The remainder of this paper uses the callback mechanism as the running example because it is the simplest to present; the polling and deferred-synchronization awaitables appear in full in the accompanying example.<sup>[9]</sup> The mechanism is a parameter of the awaitable; the protocol and the calling code are identical across all three.
