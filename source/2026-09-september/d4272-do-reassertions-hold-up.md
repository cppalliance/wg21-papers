...
This assessment tests the closure claim in P3846R1 against the public record at two cutoff dates. It rates each response for its support on the record and separately records whether the objection was resolved in one of the three forms the ISO/IEC Directives and disposition practice recognize: draft change with the commenter's confirmation, explanation whose evidence suffices for its conclusion, or recorded decision with a stated rationale. *Addressed* is P3846R1's word; *resolution* is the Directives', and what the Directives oblige is a good-faith attempt at it.
...
5. Four clarifications to the public record:<!-- lah: this nesting doesn't display properly in VS Code. Maybe you find it will render accurately for the final document. -->
...
7. The assignment of the reconciliation duty to the convenership and the absence, across the public sources this paper searched, of a reconciliation record during the adoption arc <!--lah: the last clause of this sentence is hard to parse-->read against that description. This is a question of office rather than of authorship, and Section 10 sets out both the search scope and the reasons a strong executive is the right design.<!-- lah: this is the only item in the list that is a complete sentence (other than the subitems of 5). Can you rewrite this to be a fragment? or rewrite the others to be complete sentences? -->
...
The reflector exchange over non-ignorable checks used that function directly, answering a live objection with the statement that the authors had done their best to explain the point in P3846R0, Concern 1.<sup>[54]</sup> The live objection was routed back to the paper as the existing answer. Cited only as later corroboration, the post-cutoff C++29 roadmap preserves the same bibliography, listing the C++26 objections under "concerns" and P3846R0 among the "responses."<sup>[80]</sup>

### 7.2 Acknowledgment without resolution

The clearest statement comes from the reflector answer to the cross-translation-unit problem, which agreed that the problem exists and that nobody dismisses its existence.<sup>[59]</sup> That answer attributes the problem to the C++ compilation model and argues that any feature whose semantics change at build time encounters the problem. The answer then sets out, as described in P3846, the only three ways it sees to address the problem:<sup>[59]</sup>

1. Accept the problem and standardize P2900 regardless, on the ground that it delivers value to users despite the defect.
2. Require deterministic semantics across translation units, which would stop the feature from working with existing toolchains and, on this reasoning, prevent adoption.
...
The eighteen responses defending C++26 contract assertions resolve two of the eighteen objections. Assessed against the public record at two cutoff dates, the responses come out in their authors' favor on support (two Supported, six Substantially Supported, ten Mixed, none Unsupported or Contradicted), while fifteen contain a material assertion the record does not support and four contain one the record contradicts. For the objections P3846R1's abstract describes as addressed, measured against the resolution the ISO/IEC Directives oblige the process to attempt, fifteen objections are Partly Answered and one is Unresolved; two are Answered. The gap between the two axes is the finding: The responses produced substantive argument without closing the objections they answer.
...
This paper asks for nothing. <!-- lah: I would remove this. You make this kind of statement in the second paragraph of this disclosure.-->
