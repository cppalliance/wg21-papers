...
Measured against the resolution standard the ISO/IEC Directives define, P3846R1's eighteen responses resolve two of the eighteen objections raised against C++26 contract assertions. <!-- lah: A one-sentence paragraph is counter to professional publishing standards. --> The assessment herein tests the closure claim in P3846R1 against the public record at two cutoff dates. It rates each response for its support on the record and then measures the response against the standard the Directives assign to the word the claim uses: resolution by draft change, by explanation whose evidence suffices for its conclusion, or by recorded decision with a stated rationale. 

P3846R1, "C++26 Contract Assertions, Reasserted," answers eighteen concerns about the contract assertions facility adopted into the C++26 working draft, and its abstract asserts that the objections were addressed in subsequent responses and extensively discussed in EWG. On support, the record is in the authors' favor: two responses are Supported, six Substantially Supported, ten Mixed, and none Unsupported or Contradicted, though fifteen responses contain a material assertion the record does not support, four of them contradicted by it. On resolution, the record is in favor of the objecting position: two objections Answered, fifteen Partly Answered, one Unresolved. The authors' own public statements show the distinction in operation: They direct live objections to P3846 and invoke prior votes, roadmap boundaries, timing, or future work as reasons to retain the C++26 design unchanged. 
...
5. Four corrections to the public citation record. <!-- lah: what formating are you using fornested lists? The four items below should be a nested list. --> a. The 2016 lost-optimization report is LLVM issue 28170, renumbered from Bugzilla 27796 when the LLVM tracker migrated to GitHub.<sup>[9]</sup> b. The Kona minutes N5031 are a 2025 document.<sup>[8]</sup> c. P3626R0 is the alternative wording prepared by the lead author of P2900R14 and P3846R1 so that EWG could poll the alternative; it is not an independent rival proposal.<sup>[10]</sup> d. P3097R0 was merged into P2900R8,<sup>[11]</sup> a revision of the proposal, never into the C++26 working draft.<sup>[12]</sup>
...
Three assumptions govern the assessment. First, a paper's printed date fixes what its authors could have known: A source dated after 2025-11-03 cannot retroactively falsify a sentence written by that day. <!--lah: this argument is hard to follow. Maybe an example would help. If I had a question on Monday, and on Tuesday a new paper came out that answered my question, why would Tuesday's paper somehow be prohibited in answering my question? So tell me more about the argument you are trying to make here re paper dates because right now I'm not buying one of your three assumptions. --> Second, a committee vote establishes procedural disposition without settling a technical question. Third, public sources are the only admissible evidence; a claim resting on unpublished discussion is recorded as unsourced rather than as false.

---

## 2. Resolution, Not Response, Is the Standard P3846R1 Claims
<!--lah: I don't like using "standard" in a meaning that isn't C++ Standard. Can you use a different word? Norm? Obligation? -->

The ISO/IEC Directives define consensus around resolution, not reply. The foreword principles state<sup>[13]</sup>: <!-- lah: generally, quoted material of 4 lines or more should be indented -->
...
The Unsupported Subclaim flag is a separate yes/no field, set when the response contains a specific material assertion (factual, causal, quantitative, or historical) that lacks support or is false, regardless of the complete response's rating. Resolution status answers a third question: whether the response meets the original objection on its own terms. "Answered" means the supported response meets the stated objection. "Partly Answered" means the response explains a tradeoff or documents a procedure while leaving a material technical issue open. "Unresolved" means the response does not meet the objection's central mechanism or evidence. A committee vote establishes procedural disposition; it does not by itself convert a technical status from Unresolved to Answered. The  fields <!-- which fields? All of them? Unresolved and Answered? --> are separate because combining them produces two characteristic errors: treating one incorrect sentence as the failure of an entire response and treating a committee decision as proof of a technical proposition.

The evidence standard is symmetric and limits both sides alike. Expert testimony may support a qualitative judgment when public and attributed but cannot support an unattributed quantitative comparison. A coding rule proves recognition of a risk, not frequency.<!-- lah: frequency of what? of the risk? --> Evidence made public after the build-date cutoff cannot retroactively support a printed sentence; it can support a future revision. <!-- lah: Why? If we learn more and that knowledge supports an earlier position, why would be say we can't apply that later knowledge? --> Because no published minutes record subgroup straw-poll tallies, poll tallies come from the chair-posted threads on the public cplusplus/papers tracker. The Hagenberg minutes direct readers to the tracker,<sup>[23]</sup> and the rationale paper corroborates each tally where that paper records the same poll.<sup>[24]</sup> This standard is the author's own construction, and the author has a material stake in the question under assessment, as the Disclosure section states. If the author's standard is rejected, the quoted sentences and cited artifacts stand independently of the rating scheme, and the resolution column rests on the Directives' definition rather than on the author's.

Three limitations bound what the counts <!-- lah: the tallies? --> can mean; Section 5 states concern-level caveats in place.
...
3. The ratings are judgments rather than measurements; <!-- lah: I immediately think "subjective," whch I know is not what you want readers to think --> another reviewer applying the same standard could move a boundary case by one category. <!-- lah: why just one? This places a boundary on a personal judgment.-->
...
<!-- lah: This formatting is a little strange and would benefit from rethinking. It seems like we have the status, then the verdicts, then the quote, and then more narrative justifying the verdicts. I think it should be status, quote, narrative, verdicts. And set the verdicts off somehow; right now, they are just sentence fragments plunked at the end of the paragraph. Make this material seem connected and leading somewhere instead of chunk, chunk, chunk, chunk.-->
...
<!--lah: "limit, and one resolved objection"? The commas can't do all the work here. Is this a list of three things? Is the "each" clause an appositive? -->
...
The status entry records committee time, failed consensus, and absence of new information. None <!-- lah: none what? No record? No norm? --> supplies the frequency evidence on which the response's practical dismissal depends. The rationale paper records the committee time in its own words. Of the Wroc&lstrok;aw poll on strict predicates (SF 10, F 6, N 3, A 14, SA 16): "EWG had consensus against pursuing this direction, although sustained opposition to that decision remained," with the recorded result "Consensus against, but P2900 would be in danger of failure in plenary." Of the Hagenberg poll "P2900: reduce the amount of Undefined Behavior in contracts" (SF 9, F 7, N 9, A 22, SA 19): "The group took a poll to do 'something' about this concern" ([P2899R1](https://www.open-std.org/jtc1/sc22/wg21/docs/papers/2025/p2899r1.pdf), Section 3.6.1).<sup>[24]</sup> <!-- lah: what point are you making with the last two quotes from polls? Tell me what you want me to understand about them. -->
...
The authors' own rationale fixes the baseline. On 2025-03-14, P2899R1 stated of both compilers (in Sections 2.10 and 3.5.5<sup>[24]</sup>): 
...
**Concern 3: Dependency management.** The objection, raised by P3849R0, is that contracts "introduce several new build configurations, but we have not yet seen concrete examples of how they interact with real-world build systems or complex dependency graphs."<sup>[5]</sup> The response answers with a replacement argument ("P2900 introduces no new configuration dimension," p. 11) and an example: "Boost.Build already added such support on top of the available GCC and Clang implementations of P2900. Adding this support took less than an hour of implementation effort" (p. 12). The example is genuine: the commit contains 149 additions across 9 files with a correct flag mapping,<sup>[60]</sup> and its example repository demonstrates the per-translation-unit model across the ignore-and-enforce combinations for static linking, with the shared-library case commented out from file creation.<sup>[61]</sup> The elapsed-time figure is the author's own unwitnessed report about his own work; the commit author is both the B2 maintainer and a P3846R1 coauthor, and that fact does not invalidate a public commit.

The response does not establish that implementation freedom and the Boost.Build example address complex dependency graphs. 
<!-- lah: Claude and I disgree on this one. It reads to me like  
"The response does not establish that implementation freedom, and the Boost.Build example [does not] address complex dependency graphs," thus my change to "The response does not establish that implementation freedom, nor does the Boost.Build example address complex dependency graphs." Claude reads this like "The response does not establish that [subject 1: implementation freedom] and [subject 2: the Boost.Build example] address complex dependency graphs." Which one was intended? We need some clarity in a rewrite so that the intended reading is absolutely clear. -->The package-manager case depends on a mechanism the example does not exercise: The rationale paper lists as the first use case for implementation-defined selection "[p]ackage managers and other forms of packaged software, which should not be forced to distribute a different binary for each possible evaluation semantic," resting on "the requirement that a conforming Contracts implementation should be allowed to enable and disable contract checks without requiring a rebuild," and records in the same section that only per-translation-unit configuration existed in either compiler (Section 3.5.5).<sup>[24]</sup> Per-translation-unit flag mapping is what the B2 commit demonstrates, and it is the one mechanism that does not deliver the package-manager benefit; the link-time, load-time, and runtime selection that would deliver it is the unimplemented mechanism of Concern 2. Mixed; the elapsed-time figure is a material Unsupported Subclaim. Partly Answered. <!--lah: It's strange for the author to cite their own remarks about elapsed time and then invalidate it for lack of a witness,; if it were supporting the author's argument, I would see understand this as a way to avoid the appearance of bias, but when it supports the other side, invalidating it *does* seem like bias. Maybe it would be better to clearly say that this remark is entirely thrown out since it was unwitnessed and it cannot be used to support either side of the debate. -->
...
**Concern 16: Decomposition.** The objection is that contract assertions could be composed from more primitive features standardized individually. The response identifies concrete omissions (a shared global handler, control over check injection, and the assertion marker <!-- lah: that? --> tools consume) in the proposed decomposition and states: "The idea to redesign contract assertions as a composition of more primitive features was first proposed in [P1893R0] and subsequently shown to be inadequate for the real-world use cases for contract assertions ([P1995R1])" (p. 32). The attached citation does not support the sentence: P1995R1 catalogs and polls use cases and does not mention or evaluate P1893R0.<sup>[73]</sup><sup>[74]</sup> The identified omissions in the sketch stand unanswered, and a P2900R14 coauthor who did not sign P3846R1 argues separately that an atomic assertion marker supports tooling.<sup>[75]</sup> Mixed; "shown to be inadequate" is a material Unsupported Subclaim by citation mismatch. Partly Answered.
...
<!-- lah: Seems like we're getting redundant with the argument. Isn't the concern by concern section and the table enough? --> 
...
All three of Section 2's forms occur within this dispute, and two responses meet one. <!--lah: two response meet the first form? Or two responses meet one form? --> Concern 10 supplied a working mechanism and a recorded SG21 decision declining the alternative. Concern 8 supplied a mechanism with its tradeoffs weighed on the record and seven recorded decisions declining removal. The same dispute produced a resolution by change: P3878R1 modified the draft, and its authors record that "the submitters of NB comments about standard library hardening confirmed that this change resolves their concerns."<sup>[77]</sup> Fifteen of the eighteen responses stop at explanation, and P3846R1's own text records where each stops.
...
Doumler used that function directly in the public exchange over non-ignorable checks<sup>[54]</sup>:

> Josh and I did our best to explain this in P3846R0, Concern 1.
...
The clearest statement comes from Doumler's answer to the cross-translation-unit problem<sup>[59]</sup>:

> I agree that the problem you highlighted exists. I don't think anybody dismisses the existence of the problem.

Doumler then attributes the problem to the C++ compilation model and argues that any feature whose semantics change at build time encounters it. He continues<sup>[59]</sup>:
...
The problem is acknowledged without dispute. The first listed answer is to accept it and standardize the design unchanged. Berne described the same residual outcome and placed the controls outside the standard<sup>[81]</sup>: <!--lah: indented quotes don't need quote marks; the indent is doing that work.-->
...
<!-- lah: is it acceptable to publish reflector discussion and name names? I thought not. -->
...
The rationale later described the treatment of strict predicates<sup>[24]</sup>:
...
The first group considered the topic closed; the next group showed significant interest in the same concern. The same rationale names its own category for the papers that followed: "A comprehensive response to all these concerns was published in [P3591R0]."<sup>[24]</sup> In October 2025, Doumler made his proposed evidentiary rule explicit. After writing, "It would be amazing to get actual hard data on this that could inform our opinion," and describing the discussion as "entirely subjective," he continued<sup>[54]</sup>:
...
> Procedurally, delaying the standardisation of P2900 due the concerns of [P3829R0] about interactions with hypothetical proposals would be against WG21 practice.
<!-- lah:check quote. If you quoted it wrong, fix it. If the paper itself is wrong, fix it like this: "due to [the] concerns" -->

Berne applied the same division between MVP and later work to source-level semantic control<sup>[44]</sup>:

> We've also spent a lot of time in SG21 and EWG polling what features to target as part of the initial MVP and what features should be left until later, and the Contracts feature in the working draft today is the result of that decision making.
...
Three objections to this assessment deserve their own section, each stated in its strongest form. The first is structural. Almost any response to a design objection leaves a material technical issue open, so a distribution of fifteen Partly Answered out of eighteen is a property of the rubric rather than a finding about P3846R1. The rubric never states what "Answered" would require. Objections that express priorities rather than technical questions cannot be resolved technically by construction. The second is P3846R1's own hedge: Its abstract qualifies the closure claim with "Some of the concerns are legitimate yet unavoidable with any viable assertion facility. Others reflect misunderstandings of the proposal, leading to inaccurate observations" (p. 1), so on this reading, P3846R1 never claimed to resolve all eighteen, and measuring it against a resolution requirement holds it to a standard it disclaimed. The third is the rationale paper: P2899R1<sup>[24]</sup> provides, for each section of P2900R14, "a summary of the motivation for the proposed design, a history - as complete as possible - for the design decisions in that section," <!--lah: if you're quoting and the quoted material uses em dashes, use actual em dashes. We don't get to apply house style to others' works. --> together with "polls that were taken and their results as called by the chair at the time" (Section 1), so on this reading Section 2's third form, a recorded decision with a stated rationale, already exists for every design decision the objections attack, and this paper <!-- lah: 2899? 2900? 3846? THIS paper? --> cites that document eight times for tallies while denying it the status it claims. <!-- lah: too many vague "it" uses-->

The standard is not the rubric's. <!-- lah: too much ambiguous language. We have a description of what various entities require. We have the meaurement system in this paper. So what is "the standard" here and what is "the rubric"? Give each piece a name and use that name consistently. --> Section 2's three forms come from the ISO/IEC Directives and from disposition practice rather than from the three fields (Overall Support, Unsupported Subclaim, and Resolution) of Section 3. A response resolves <!-- lah: Go back to Section 2 and reread those quotes re resolution. What does it mean to "seek," to "try," and to "attempt" to reach resolution? --> an objection by changing the draft with the commenter's confirmation, by an explanation whose evidence suffices for its conclusion, or by a recorded decision with a stated rationale. The rubric's definition of "Answered," the supported response meets the stated objection on its own terms, is the same test stated for a single response.
...
4. **The polls themselves.** P2899R1 characterizes two of the Hagenberg polls on which P3846R1's status entries rest. Of "P2900: add ODR to contracts," it states, "EWG took a deliberately vaguely worded poll to gauge the interest of the room to 'do something about it'" (Section 3.5.11); of "P2900: reduce the amount of Undefined Behavior in contracts," it says, "The group took a poll to do 'something' about this concern" (Section 3.6.1).<sup>[24]</sup> Those are the decisions behind Concerns 4, 7, and 16. A poll the proposers describe as deliberately vague, taken to gauge interest, is not a recorded decision with a stated rationale on any objection.

The abstract's hedge does not exempt the closure claim either. Its two classes are unnamed: No concern is mapped to "legitimate yet unavoidable" or to "misunderstanding," and no recorded decision disposes of any concern on either ground, so the hedge reserves an exemption without claiming it for anything. "Unavoidable with any viable assertion facility" is a universal claim about every viable facility, and the record contains a polled candidate counterexample: P3626R0, the alternative wording the lead author prepared so that EWG could poll the choice.<sup>[10]</sup> The "misunderstandings" class has its own form under Section 2, resolution by explanation, and the fifteen flagged sentences of Table 2 are explanations whose evidence is inadequate for their conclusions; a misunderstanding is not corrected by an unsupported statement. "Extensively discussed" names the floor clause 2.6.5 sets and does not name the resolution the same clause obliges. <!-- lah: Again, the obligation is *trying* to achieve resolution. -->

The rubric does not punish candor. The columns answer different questions by construction: Candor about limits is rewarded in the support column, where the bounded Concern 5 response is Supported with no flag. The resolution column records the objection's state, not the response's virtue; a candidly acknowledged open issue is still an open issue, and reclassifying it as closed would be the inaccuracy <!-- lah: "dishonesty" implies intent. If I'm not mistaken the Code of Conduct has a clause about assuming good intent. --> the support column exists to catch.

The boundary cases do not decide the finding. If every contested boundary moved one category in the authors' favor (Concern 11 to Partly Answered and Concerns 5 and 18 to Answered), the distribution becomes four Answered and fourteen Partly Answered. The conclusion survives the most favorable defensible reading: <!-- lah: is that the most favorable? What if I said they should move two categories and I had reasons to defend my judgments? --> The responses produced substantive argument without closing the objections they answer. The distribution is, therefore, a property of the responses rather than of the instrument.
...
This paper assesses P3846R1's eighteen responses against the public record at the two cutoffs stated in Section 3, a record broader than the sources P3846R1 itself cites. This paper asks for nothing; it proposes no wording and requests no poll. Section 9 describes practice in force in other bodies and in one WG21 subgroup and requests none of it. <!-- lah: I find this insincere. Perhaps the request is implied or implicit. Or maybe the intent is just to put this assessment on the record. That's not technically an "ask," but the "this paper asks for nothing" line comes across as the same kind of not-exactly-the-whole-story that the paper is taking 3846 to task for. -->
...
The ISO/IEC provisions cited in Section 2 bind the national body ballot and plenary level of the standards process, not working group papers; Section 2 states the two reasons the resolution definition nonetheless supplies the test for P3846R1's closure claim. One further limitation of the method is the author's own: The assessment tests the sentences the author selected as load-bearing in each response, and a different selection could produce a different distribution of subclaim flags even if every verdict here were upheld. <!-- lah: This caveat seems honest. -->
