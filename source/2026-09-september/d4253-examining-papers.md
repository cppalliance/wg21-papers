...
3. [exec.let] contains no prose statement of when the predecessor operation state ends. The lifetime is specified by the `emplace<monostate>()` line alone.<sup>[12]</sup>
4. P3373R4's wording names the variant type `ops2_variant_t` in three places and `ops_variant_t` in one.<sup>[1]</sup> The editorial application, draft commit `27bd4d47`, normalized the name to `ops_variant_t` and left [exec.let]/14 reading "the types args_variant_t and ops2_variant_t are well-formed."<sup>[24]</sup><sup>[12]</sup>
5. P3373R4 reports: "This change has been implemented in nVidia's stdexec."<sup>[1]</sup> The merged code invokes the factory and then destroys the predecessor;<sup>[25]</sup><sup>[26]</sup> the adopted wording destroys the predecessor and then invokes the factory.<sup>[12]</sup> libunifex, the cited existing practice, follows the wording's order: "we need to destroy predOp_ first to make room", then the factory.<sup>[27]</sup>
...
No field report is cited here, and none was found: ten searches of the stdexec and libunifex issue trackers for `let_value` with `dangling`, `lifetime`, `use-after-free`, `destroyed`, and `ASAN` returned no report of this shape. The nearest items are stdexec #282, which asks why libunifex destroys the successor state and values before completing;<sup>[30]</sup> stdexec #1076, which asks "Is it always safe to release operation states early or does it lead to issues elsewhere?";<sup>[31]</sup> and libunifex #416, an AddressSanitizer fix in `let_value` for coroutine-evaluated values.<sup>[32]</sup> The claim is about what the adopted wording permits. P3373R4's own argument about latent issues, that code tested against one set of algorithms is later composed with another "with less rigor", applies to it.<sup>[1]</sup>
