...
struct callback
{
    static_assert(memfn_size >= sizeof(int*), 
        "member function pointer size must be at least pointer size");

    int* count_ptr_;
    std::byte padding_[memfn_size - sizeof(int*)];

    explicit callback(int& count) noexcept
        : count_ptr_(&count)
    {
...
static_assert(sizeof(callback) == memfn_size, 
    "callback size must match member function pointer size");
